import numpy as np
from layers.utils import tanh, sigmoid, sigmoid_grad, tanh_grad
from layers.utils import initalize, zeros, ones
...
class Lstm(object):
    ...
    def forward(self, x_t):
        ...
        self.ouput_gate[t] = sigmoid(np.dot(self.W_ho, h) + np.dot(self.W_xo, x_t) + self.b_o)
        self.cell_update[t] = sigmoid(np.dot(self.W_hj, h) + np.dot(self.W_xj, x_t) + self.b_j)
        
        self.c[t] = self.input_gate[t] * self.cell_update[t] + self.forget_gate[t] * self.c[t-1]
        self.ct[t] = tanh(self.c[t])
        self.h[t] = self.ouput_gate[t] * self.ct[t]
